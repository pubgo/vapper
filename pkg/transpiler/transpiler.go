					...
					g.Qual("github.com/gopherjs/vecty", "Markup").CustomFunc(call, func(g *jen.Group) {
						for _, v := range token.Attr {
							...
							case transpiler.BoolProps[v.Name.Local] != "":
								value := v.Value == "true"
								g.Qual("github.com/gopherjs/vecty/prop", transpiler.BoolProps[v.Name.Local]).Call(
									jen.Lit(value),
								)
							case transpiler.StringProps[v.Name.Local] != "":
								if strings.HasPrefix(v.Value, "{vecty-field:") {
									...
									field = field[:len(field)-1]
									g.Qual("github.com/gopherjs/vecty/prop", transpiler.StringProps[v.Name.Local]).Call(
										jen.Id("p").Dot(field),
									)
								} else {
									g.Qual("github.com/gopherjs/vecty/prop", transpiler.StringProps[v.Name.Local]).Call(
							...
							case v.Name.Local == "type" && transpiler.TypeProps[v.Value] != "":
								g.Qual("github.com/gopherjs/vecty/prop", "Type").Call(
									jen.Qual("github.com/gopherjs/vecty/prop", transpiler.TypeProps[v.Value]),
								)
							default:
